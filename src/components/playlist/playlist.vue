<template>
  <transition name="list-fade">
    <div class="playlist" v-show="showFlag" @click="hide">
      <div class="list-wrapper" @click.stop>
        <div class="list-header">
          <h1 class="title">
            ...
            <span class="clear" @click="showConfirmBox">
        ...
        <scroll :refreshDelay="refreshDelay" class="list-content" ref="listContent" :data="sequenceList">
          <transition-group name="list" tag="ul">
            <li :key="item.id"
              ...
              <span class="like" @click.stop="favourSong(item)">
      ...
      <add-song ref="addSong" @select="hasSelect"></add-song>
    </div>
  </transition>
</template>
    ...
    methods: {
      ...
      showPlaylist() {
        ...
        this.$nextTick(() => {
          ...
          // 点击歌单进入自己的播放列表，显示当前播放歌曲
          this.scrollTo(this.currentSong)
        })
      },
      hide() {
        this.showFlag = false
      ...
      scrollTo(currentSong) {
        const index = this.sequenceList.findIndex((song) => {
          return currentSong.id === song.id
        })
        this.$refs.listContent.scrollToElement(this.$refs.currentItem[index], 320)
      },
      deleteOne(item) {
